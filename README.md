...
```bash
lark2wechat publish <飞书文档链接> --theme zenfox        # 端到端 → 草稿箱（默认主题）
lark2wechat render  <md文件>      --theme zenfox          # 只渲染预览 HTML
...
| 主题 | 风格 |
|---|---|
| `zenfox`（默认） | 暖灰墨正文+砖红衬线标题+橘红强调+红色色条 |
| `warm` | 暖色出版，衬线标题×无衬线正文，奶油底 |
| `feishu` | 飞书文档阅读视图（飞书蓝+浅灰块+高亮块） |
